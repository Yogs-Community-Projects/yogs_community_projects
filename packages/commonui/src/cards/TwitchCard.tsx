...
const LastStreamCard: Component = () => {
  ...
  return (
    <a
      class="bg-twitch hover:scale-101 hover:brightness-102 my-auto flex aspect-[8/1] w-full flex-row items-center rounded-2xl p-2 text-white transition-all"
    ...
    >
      ...
      <div class="my-auto ml-2 flex-1">
        ...
        <div class="text-sm">
          Last Live {DateTime.fromISO(lastStream!.endedAt).toLocaleString(DateTime.DATE_MED_WITH_WEEKDAY)}
        </div>
        <div class="text-sm">{DateTime.fromISO(lastStream!.endedAt).toLocaleString(DateTime.TIME_SIMPLE)}</div>
      </div>
    </a>
  )
}
