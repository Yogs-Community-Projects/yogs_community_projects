...
const DayButtons: Component = () => {
  ...
  return (
    <div class={'p-schedule h-data'}>
      <div class={'schedule-card-white flex flex-row justify-center items-center'}>
        <button class={'w-data ripple rounded-2xl flex flex-col justify-center items-center'} onclick={prev}>
          <FaSolidChevronLeft/>
        </button>
        <p class={'flex-1 text-day-header text-center'}>
          {DateTime.fromJSDate(new Date(useCurrentDay().start)).toFormat("EEE',' MMM d")}
        </p>
        <button class={'w-data ripple rounded-2xl flex flex-col justify-center items-center'} onclick={next}>
          <FaSolidChevronRight/>
        </button>
      </div>
    </div>
