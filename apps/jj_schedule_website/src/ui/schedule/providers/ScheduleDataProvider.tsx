...
export const ScheduleDataProvider: ParentComponent<{ scheduleData: ScheduleData }> = (props) => {
  return (
    <ScheduleContext.Provider value={{schedule: props.scheduleData}}>
      {props.children}
    </ScheduleContext.Provider>
  );
}
const useScheduleDataContext = () => useContext(ScheduleContext)!;
export const useScheduleData: Accessor<ScheduleData> = () => useContext(ScheduleContext)!.schedule;
...
export const useDays: Accessor<Day[]> = (): Day[] => useWeeks().map((week) => week.days).reduce((a, b) => a.concat(b));
...
export const useSlots: Accessor<Slot[]> = (): Slot[] => useDays().map((day) => day.slots).reduce((a, b) => a.concat(b));
export const useMaxDayCount: Accessor<number> = (): number => useWeeks().map((week) => week.days.length).reduce((a, b) => Math.max(a, b))
export const useMaxSlotCount: Accessor<number> = (): number => useDays().map((day) => day.slots.length).reduce((a, b) => Math.max(a, b))
export const useTimes: Accessor<Time[]> = () => useCurrentWeek().times ?? useScheduleData().schedule.times;


export const useCreatorIds: Accessor<string[]> = () => {
  return [...new Set(useSlots().map((slot) => slot.relations.creators)
