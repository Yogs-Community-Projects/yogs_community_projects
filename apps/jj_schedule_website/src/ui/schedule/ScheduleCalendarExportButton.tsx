...
const CalendarDialogDialogBody: Component<CalendarDialogDialogBodyProps> = props => {
  ...
  const creatorNames = (slot: Slot) => {
    ...
    return slot.relations.creators.map((id) => map.get(id))
      .filter(c => c !== undefined).map(c => c!.creator.name);
  }
  const download = () => {
    const s = icalString()
  ...
  const isInSearch = (slot: Slot) => {
    ...
    return slot.title.toLowerCase().includes(search().toLowerCase()) ||
    ...
    <div class={'flex h-full w-full flex-col rounded-3xl bg-white'}>
                          ...
                          <Show when={isInSearch(slot)}>
                            ...
                            >
                              <div class={'flex flex-row items-center'}>
                                <p
                                  class={'flex-1 text-left'}>{slot.title} ({start.toLocaleString(DateTime.TIME_WITH_SHORT_OFFSET)})</p>
      ...
      <div class={'flex h-[72px] flex-row items-center justify-end gap-2 p-4'}>
        ...
        <button
          ...
          class={'bg-primary rounded-xl p-2 text-white disabled:opacity-75 disabled:bg-gray-500'}
          onclick={download}>
          Download
        </button>
      </div>
